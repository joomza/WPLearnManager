 ...
 * oauth_client.php
 *
 * @(#) $Id: oauth_client.php,v 1.146 2016/03/30 04:04:43 mlemos Exp $
 *
 */
...
/*
  ...
  <version>@(#) $Id: oauth_client.php,v 1.146 2016/03/30 04:04:43 mlemos Exp $</version>
...
class oauth_client_class {
    ...
    var $oauth_user_agent = 'PHP-OAuth-API (http://www.phpclasses.org/oauth-api $Revision: 1.146 $)';
    var $response_time = 0;
    var $session = '';
    ...
    Function SetupSession(&$session) {
        if (strlen($this->session) || IsSet($_COOKIE[$this->session_cookie])) {
            if ($this->debug)
                $this->OutputDebug(strlen($this->session) ? 'Checking OAuth session ' . $this->session : 'Checking OAuth session from cookie ' . sanitize_key($_COOKIE[$this->session_cookie]));
            if (!$this->GetOAuthSession(strlen($this->session) ? $this->session : sanitize_key($_COOKIE[$this->session_cookie]), $this->server, $session))
    ...
    Function GetStoredState(&$state) {
        ...
        if (IsSet($_SESSION['OAUTH_STATE']))
            $state = sanitize_key($_SESSION['OAUTH_STATE']);
        else{
            $_SESSION['OAUTH_STATE'] = time() . '-' . substr(md5(rand() . time()), 0, 6);
			$state = sanitize_key($_SESSION['OAUTH_STATE']);
		}
        return(true);
    }
    ...
    Function GetRequestState(&$state) {
        ...
        $state = (IsSet($_GET[$check]) ? sanitize_key($_GET[$check]) : null);
        return(true);
    }
    ...
    Function GetRequestError(&$error) {
        $error = (IsSet($_GET['error']) ? sanitize_key($_GET['error']) : null);
        return(true);
    }

    Function GetRequestDenied(&$denied) {
        $denied = (IsSet($_GET['denied']) ? sanitize_key($_GET['denied']) : null);
        return(true);
    }

    Function GetRequestToken(&$token, &$verifier) {
        $token = (IsSet($_GET['oauth_token']) ? sanitize_key($_GET['oauth_token']) : null);
        $verifier = (IsSet($_GET['oauth_verifier']) ? sanitize_key($_GET['oauth_verifier']) : null);
        return(true);
    }
    ...
    Function GetAccessToken(&$access_token) {
        ...
        if (IsSet($_SESSION['OAUTH_ACCESS_TOKEN'][$access_token_url]))
            $access_token = sanitize_key($_SESSION['OAUTH_ACCESS_TOKEN'][$access_token_url]);
        else
            $access_token = array();
        return true;
    }
            ...
            <html>
                ...
                <body>
                    ...
                    if (strlen($this->authorization_error)) {
                        ...
                        if (strlen($this->access_token_expiry)) {
                            ?>
                            <p>Access token expiry: <?php echo esc_attr($this->access_token_expiry); ?> UTC</p>
                            <?php
                        }
                    }
                    ?>
