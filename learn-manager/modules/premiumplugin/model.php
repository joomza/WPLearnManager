...
class JSLEARNMANAGERpremiumpluginModel {
    ...
    function verfifyAddonActivation($addon_name){
        ...
        $transaction_key = $GLOBALS['wpdb']->get_var( "SELECT `option_value` FROM " . jslearnmanager::$_wpprefixforuser . "options WHERE option_name = '$option_name'"); 
   ...
   public static function activate( $args ) {
    ...
    function getAddonSqlForUpdation($plugin_slug,$installed_version,$new_version){
        $option_name = 'transaction_key_for_learn-manager-'.$plugin_slug;
        $transaction_key =  $GLOBALS['wpdb']->get_var( "SELECT `option_value` FROM " . jslearnmanager::$_wpprefixforuser . "options WHERE option_name = '$option_name'");
        // get_option($option_name);
        $defaults = array(
                    ...
                    if (is_resource($file) === true) {
                        ...
                        while (feof($file) === false) {
                            ...
                            if (preg_match('~' . preg_quote($delimiter, '~') . '\s*$~iS', end($query)) === 1) {
                                ...
                                $query = str_replace("#__", jslearnmanager::$_db->prefix, $query);
                                if (!empty($query)) {
                                    jslearnmanager::$_db->query($query);
                                }
                            }
    ...
    function getMessagekey(){
        $key = 'premiumplugin';if(is_admin()){$key = 'admin_'.$key;}return $key;
    }
